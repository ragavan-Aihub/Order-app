-- Safe to run more than once. Adds 6-digit order numbers and refreshes the API.
...
create unique index if not exists orders_business_order_number_idx
...
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'orders_assign_order_number'
